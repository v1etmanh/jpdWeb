...
class CourseLearningServiceGetCourseByIdTest {
    ...
    void shouldThrowException_whenUserDoesNotHaveAccessToCourse() {
        ...
        // validationResources trả về null (giả lập user không có course)
        ...
        assertFalse(
                ex.getMessage() == null || ex.getMessage().toLowerCase().contains("not your course") || ex.getMessage().toLowerCase().contains("not exist"),
                "Thông điệp lỗi phải phù hợp với UnauthorizedException"
        );
