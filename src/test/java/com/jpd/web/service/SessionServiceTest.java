...
//
//        // Mock ValidationResources to avoid NullPointerException
//        com.jpd.web.service.utils.ValidationResources validationResources = mock(com.jpd.web.service.utils.ValidationResources.class);
//        // Use doAnswer instead of doNothing for void method, or alternatively, let the mock use default behavior
//        // (By default, mocks of void methods do nothing.)
//        // So this line can be omitted: doNothing().when(validationResources).validateCreatorExists(anyLong());
//        // (But to be explicit, you could do below:)
//        doAnswer(invocation -> null).when(validationResources).validateCreatorExists(anyLong());
//
//        // Inject the mock into the service using reflection, since it's probably a package-private field
//        java.lang.reflect.Field field = service.getClass().getDeclaredField("validationResources");
//        field.setAccessible(true);
//        field.set(service, validationResources);
//
//        // The method createSession likely requires both createSessionRequest and kahootId, fix method call
//        CreateSessionResponse resp = service.createSession(createSessionRequest, kahootId);
//
//        assertNotNull(resp);
//        assertEquals(6, resp.getSessionCode().length());
//        assertFalse(resp.getSessionId() == null || resp.getSessionId().isEmpty());
...
//    }
    ...
    void TC_ES_01_createSession_kahootNotFound() {
        ...
        RuntimeException ex = assertThrows(RuntimeException.class, () -> service.createSession(createSessionRequest, kahootId));
